...
build:
	docker run -i -t --rm --name altcomphsr.vshsr.ch -u jekyll -v $(shell pwd):/src/ -p 4000:4000 altcomphsr/altcomphsr.vshsr.ch jekyll build

serve:
	docker run -i -t --rm --name altcomphsr.vshsr.ch -u jekyll -v $(shell pwd):/src/ -p 4000:4000 altcomphsr/altcomphsr.vshsr.ch jekyll serve
