...
build:
	docker run -i -t --rm --name www.openhsr.ch -v $(shell pwd):/src/ -p 4000:4000 openhsr/www.openhsr.ch jekyll build

serve:
	docker run -i -t --rm --name www.openhsr.ch -v $(shell pwd):/src/ -p 4000:4000 openhsr/www.openhsr.ch jekyll serve
